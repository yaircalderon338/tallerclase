...
app.post('/api/estudiantes', async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO estudiantes (nombre, correo) VALUES ($1, $2)', [nombre, correo]);
...
app.delete('/api/estudiantes/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: `🗑️ Estudiante con ID ${id} eliminado` });
    } catch (error) {
        console.error(error);
...
app.post('/api/cursos', async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO cursos (nombre, creditos) VALUES ($1, $2)', [nombre, creditos]);
...
app.delete('/api/cursos/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: `🗑️ Curso con ID ${id} eliminado` });
    } catch (error) {
        console.error(error);
...
app.post('/api/inscripciones', async (req, res) => {
    ...
    try {
        ...
        if (existente.rows.length > 0) {
            return res.status(400).json({ message: '⚠️ El estudiante ya está inscrito en este curso' });
        }
...
app.delete('/api/inscripciones/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: `🗑️ Inscripción con ID ${id} eliminada` });
    } catch (error) {
        console.error(error);
...
app.get('/api/inscripciones/detalles', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: '❌ Error al obtener detalles de inscripciones', error: error.message });
    }
});
